...
describe('git module', () => {
  ...
  describe('createWorktree', () => {
    it('Case 1: worktree exists with prior work — resume', async () => {
      ...
      mockExecFileSync.mockImplementation(
        (_cmd: string, args: string[] | unknown[], _opts?: unknown) => {
          ...
          if (argsArray.includes('rev-list') && argsArray.includes('--count')) {
            return '2';
          }
          return '';
        }
    ...
    it('Case 2: worktree exists, no work — recreate fresh', async () => {
      ...
      const hasWorktreeRemove = calls.some(
        (c) =>
          Array.isArray(c[1]) &&
          c[1].includes('worktree') &&
          c[1].includes('remove')
      );
      const hasBranchDelete = calls.some(
        (c) =>
    ...
    it('Case 3: branch has work but worktree gone — adds worktree from existing branch', async () => {
      ...
      const calls = mockExecFileSync.mock.calls;
      const hasMkdir = calls.some(
        (c) =>
          c[0] === 'mkdir' &&
          Array.isArray(c[1]) &&
          c[1].includes('/tmp/worktrees')
      );
      const hasWorktreeAdd = calls.some(
        (c) =>
          Array.isArray(c[1]) &&
          c[1].includes('worktree') &&
          c[1].includes('add')
