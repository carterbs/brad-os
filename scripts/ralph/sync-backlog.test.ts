...
import { runSyncBacklog } from './sync-backlog.js';

const { mockResolveConfig, mockSyncTaskFilesFromLog } = vi.hoisted(() => ({
...
describe('sync-backlog script', () => {
  ...
  describe('logs sync results with zero removals', () => {
    ...
    it('does NOT list any tasks when arrays are empty', async () => {
      ...
      const taskLines = calls.filter(
        (c) => c.includes('  backlog:') || c.includes('  triage:')
      );
  ...
  describe('output format', () => {
    it('displays merged tasks seen line first', async () => {
      ...
      const calls = consoleLogSpy.mock.calls.map((c) => c[0]);
      const mergedIndex = calls.findIndex((c) =>
        c.includes('Merged tasks seen')
      );
