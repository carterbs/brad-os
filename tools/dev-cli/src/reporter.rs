...
/// Print the pass/fail summary for all check results and return `true` when all checks pass.
pub fn print_summary(results: &[CheckResult], total_elapsed_secs: u64) -> bool {
    ...
    for result in results {
        if result.exit_code == 0 {
            ...
            writeln!(
                ...
                "\u{2717}".red(),
                result.name,
                format!("{}s  \u{2192} .validate/{}.log", result.elapsed_secs, result.name)
                    .dimmed(),
            )
            .ok();
            all_passed = false;
    ...
    if all_passed {
        ...
        writeln!(
            ...
            "  {} {}",
            "FAIL".red().bold(),
            format!("({}s)  Logs: .validate/*.log", total_elapsed_secs)
                .dimmed(),
        )
        .ok();
    }
