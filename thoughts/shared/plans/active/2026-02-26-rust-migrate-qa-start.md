...
- Script is a workflow engine with high branching and stateful orchestration: `scripts/qa-start.sh:20`, `scripts/qa-start.sh:373`, `scripts/qa-start.sh:459`.
- It is invoked by npm via advanced env startup: `package.json:50`.
- It persists a state contract consumed by other scripts: `scripts/qa-start.sh:537`, `scripts/qa-build.sh:78`, `scripts/qa-launch.sh:78`, `scripts/qa-stop.sh:100`.
...
## Key Discoveries
- Simulator lock ownership semantics and cleanup-on-error are central invariants: `scripts/qa-start.sh:39`, `scripts/qa-start.sh:193`.
- Dynamic Firebase config generation currently embeds Node inline; this can be replaced with typed JSON manipulation in Rust: `scripts/qa-start.sh:383`.
- Process startup and readiness polling must preserve timeout behavior/log tails: `scripts/qa-start.sh:491`, `scripts/qa-start.sh:518`.
...
- Contract tests cover all CLI options from `scripts/qa-start.sh:25`.
...
## References
- `scripts/qa-start.sh`
- `docs/guides/isolated-qa-loop.md`
- `package.json`
