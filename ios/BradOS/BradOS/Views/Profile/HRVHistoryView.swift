    ...
    private func projectionSection(projected: Double, weeklyRate: Double) -> some View {
        VStack(alignment: .leading, spacing: Theme.Spacing.space4) {
            ...
            VStack(spacing: 0) {
                // Current rate
                HStack {
                    Image(systemName: weeklyRate > 0 ? "arrow.up.right" : "arrow.down.right")
                        .font(.system(size: Theme.Typography.iconMD))
                        .foregroundStyle(weeklyRate > 0 ? Theme.success : Theme.warning)
                        .frame(width: Theme.Dimensions.iconFrameMD, height: Theme.Dimensions.iconFrameMD)
                        .background((weeklyRate > 0 ? Theme.success : Theme.warning).opacity(0.12))
                    ...
                    VStack(alignment: .leading, spacing: Theme.Spacing.space1) {
                        Text("Current Rate")
                            .font(.subheadline)
                            .foregroundStyle(Theme.textPrimary)
                        Text(String(format: "%.1f ms/week", abs(weeklyRate)))
                            .font(.caption)
                            .monospacedDigit()
                            .foregroundStyle(Theme.textSecondary)
                    }
                ...
                HStack {
                    ...
                    VStack(alignment: .leading, spacing: Theme.Spacing.space1) {
                        Text("Projected HRV")
                            .font(.subheadline)
                            .foregroundStyle(Theme.textPrimary)
                        Text(String(format: "~%.0f ms in 2 weeks", projected))
                            .font(.caption)
                            .monospacedDigit()
                            .foregroundStyle(Theme.textSecondary)
                    }
