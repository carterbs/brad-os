...
describe('stretching.schema', () => {
  describe('completedStretchSchema', () => {
    ...
    it('accepts all valid body regions', () => {
      ...
      for (const region of regions) {
        const result = completedStretchSchema.safeParse({ ...validCompletedStretch, region });
        expect(result.success).toBe(true);
      }
    });
    ...
    it('rejects invalid duration seconds values', () => {
    ...
    it('rejects invalid skipped segment counts', () => {
      const negative = completedStretchSchema.safeParse({
        ...validCompletedStretch,
        skippedSegments: -1,
      });
      const tooHigh = completedStretchSchema.safeParse({
        ...validCompletedStretch,
        skippedSegments: 3,
      });
      const fractional = completedStretchSchema.safeParse({
  ...
  describe('createStretchSessionSchema', () => {
    ...
    it('rejects negative or non-integer aggregate counters', () => {
    ...
    it('rejects empty stretch arrays', () => {
      const result = createStretchSessionSchema.safeParse({
        ...validSessionPayload,
        stretches: [],
      });
