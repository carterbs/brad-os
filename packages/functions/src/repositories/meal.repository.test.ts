...
describe('MealRepository', () => {
  ...
  describe('update', () => {
    ...
    it('should return existing meal when no updates provided', async () => {
      ...
      (mockDocRef.get as ReturnType<typeof vi.fn>).mockResolvedValue(
        createMockDoc('meal-1', existingData)
      );
    ...
    it('should update effort', async () => {
      ...
      (mockDocRef.get as ReturnType<typeof vi.fn>)
        .mockResolvedValueOnce(createMockDoc('meal-1', existingData))
        .mockResolvedValueOnce(
          createMockDoc('meal-1', { ...existingData, effort: 8 })
        );
    ...
    it('should update has_red_meat', async () => {
      ...
      (mockDocRef.get as ReturnType<typeof vi.fn>)
        .mockResolvedValueOnce(createMockDoc('meal-1', existingData))
        .mockResolvedValueOnce(
          createMockDoc('meal-1', { ...existingData, has_red_meat: true })
        );
  ...
  describe('updateLastPlanned', () => {
    ...
    it('should clear last_planned timestamp', async () => {
      const repository = new MealRepository(mockDb as Firestore);
      const existingData = {
        name: 'Chicken Stir Fry',
        meal_type: 'dinner',
        ...
        prep_ahead: false,
        url: '',
        last_planned: '2024-06-15T00:00:00Z',
        created_at: '2024-01-01T00:00:00Z',
        updated_at: '2024-01-01T00:00:00Z',
      };
      ...
      (mockDocRef.get as ReturnType<typeof vi.fn>)
        .mockResolvedValueOnce(createMockDoc('meal-1', existingData))
        .mockResolvedValueOnce(
          createMockDoc('meal-1', { ...existingData, last_planned: null })
        );

      const result = await repository.updateLastPlanned('meal-1', null);
    ...
    it('should return null when updating last_planned for non-existent meal', async () => {
      ...
      const result = await repository.updateLastPlanned(
        'non-existent',
